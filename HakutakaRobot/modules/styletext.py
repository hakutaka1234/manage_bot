from HakutakaRobot import dispatcher
...
def haku(update, context):
    ...
    if not string:
        message.reply_text("Usage is `/haku <text>`", parse_mode=ParseMode.MARKDOWN)
        return

    for normiecharacter in string:
        if normiecharacter in normiefont:
            hakucharacter = hakufont[normiefont.index(normiecharacter)]
            string = string.replace(normiecharacter, hakucharacter)
...
haku_HANDLER = DisableAbleCommandHandler("haku", haku, run_async=True)
...
dispatcher.add_handler(haku_HANDLER)
