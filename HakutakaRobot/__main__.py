...
import sys
import traceback
import HakutakaRobot.modules.sql.users_sql as sql
from sys import argv
from typing import Optional
from telegram import __version__ as peler
from platform import python_version as memek
from HakutakaRobot import (
    ALLOW_EXCL,
    CERT_PATH,
    DONATION_LINK,
    BOT_USERNAME as bu,
...
from HakutakaRobot.modules import ALL_MODULES
from HakutakaRobot.modules.helper_funcs.chat_status import is_user_admin
from HakutakaRobot.modules.helper_funcs.misc import paginate_modules
...
PM_START_TEXT = """
*Hello {} !*
㊝ I'am Robot [㊝](https://graph.org/file/a4d8ec008094e0e71cbda.jpg)
────────────────────────
× *Uptime:* `{}`
× `{}` *users, across* `{}` *chats.*
────────────────────────
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("HakutakaRobot.modules." + module_name)
...
def haku_about_callback(update, context):
    query = update.callback_query
    if query.data == "haku_":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="Admins", callback_data="haku_admin"),
                    InlineKeyboardButton(text="Notes", callback_data="haku_notes"),
                 ],
                 [
                    InlineKeyboardButton(text="Support", callback_data="haku_support"),
                    InlineKeyboardButton(text="Credits", callback_data="haku_credit"),
                 ],
                 [
                    InlineKeyboardButton(text="Source Code", url="https://github.com"),
                 ],
                 [
                    InlineKeyboardButton(text="Go Back", callback_data="haku_back"),
                 ]
                ]
            ),
        )
    elif query.data == "haku_back":
        first_name = update.effective_user.first_name
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
                PM_START_TEXT.format(
        ...
        query.message.edit_text(
            ...
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(text="Go Back", callback_data="haku_")]]
            ),
        )

    elif query.data == "haku_notes":
        query.message.edit_text(
    ...
    elif query.data == "haku_support":
        query.message.edit_text(
            text="*๏ haku support chats*"
            f"\nJoin My Support Group/Channel for see or report a problem on {dispatcher.bot.first_name}.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="Support", url="t.me/hakuSupport"),
                    InlineKeyboardButton(text="Updates", url="https://t.me/hakuProject"),
                 ],
                 [
                    InlineKeyboardButton(text="Go Back", callback_data="haku_"),
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(
        haku_about_callback, pattern=r"haku_", run_async=True
    )
