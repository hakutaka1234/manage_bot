...
from HakutakaRobot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./HakutakaRobot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Grey Cililk](https://t.me/HakutakaRobot_bot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./HakutakaRobot/resources/fonts"):
            os.remove(font_)
...
async def logo_(event):
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [HakutakaRobot](https://t.me/HakutakaRobot_bot)",
            force_document=False,
        )
        os.remove(flnme)
        await xx.delete()
...
async def logo_gen(event):
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [HakuRobot](https://t.me/HakutakaRobot)",
            force_document=False,
        )
        os.remove(flnme)
        await xx.delete()
